...
const requiredKeys = trackedKeys;
...
const AddStudent = () => {
  ...
  const requiredReady = requiredKeys.every((key) => String(formValues?.[key] || '').trim());
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      navigate('/students', { state: { toast: 'Student added successfully.' } });
    } catch (err) {
    ...
    <div className="max-w-6xl mx-auto space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-slate-500 text-sm">Create a student profile and login account</p>
        </div>
      </div>
          ...
          <div className="p-5 space-y-5">
            <div className="flex items-start gap-4">
              ...
              <div className="min-w-0">
                ...
                <span className={`inline-flex mt-2 px-2.5 py-1 rounded-lg text-xs font-semibold ${
                  requiredReady ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'
                }`}>
                  {requiredReady ? 'Ready to save' : 'Required details pending'}
                </span>
              </div>
            </div>
